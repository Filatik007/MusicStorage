...
function musicCreator(music) {
    let element = document.createElement("div");
    element.setAttribute("class", "musicBlock");
    let imgBlock = document.createElement("div");
    imgBlock.setAttribute("class", "imageBlock");
    let img = document.createElement("img");
    if (music.image.includes("base64"))
        img.setAttribute("src", music.image);
    else
        img.setAttribute("src", "./src/img/" + music.image);
    ...
    if (music.song) {
        ...
        if (music.song.includes("base64"))
            audio.setAttribute("src", music.song);
        else
            audio.setAttribute("src", "./src/music/" + music.song);
...
function init() {
    ...
    for (let key in localStorage) {
        ...
        let musicElement;
        if (stringMusic)
            musicElement = JSON.parse(stringMusic);
        if (musicElement)
            music.push(musicElement);
    }
}
function refresh() {
    if (musicBlock)
    ...
    actualMusic.forEach(element => {
        musicBlock === null || musicBlock === void 0 ? void 0 : musicBlock.appendChild(musicCreator(element));
    });
}
function sort() {
...
function filterYesNoCreator(name, property) {
    ...
    filterBlock === null || filterBlock === void 0 ? void 0 : filterBlock.appendChild(element);
}
function filterCreator(name, property) {
    let filterBlock = document.getElementById("filters");
    let element = document.createElement("div");
    element.setAttribute("class", "filterElement");
    let label = document.createElement("label");
...
function filterProcess() {
    ...
    selects.forEach(element => {
        ...
        if (selectedValue == "true") {
            actualMusic = actualMusic.filter(music => music[selectedProp]);
        }
            ...
            actualMusic = actualMusic.filter(music => {
                let musicValue = music[selectedProp];
                if (Array.isArray(musicValue))
                    return musicValue.includes(selectedValue);
                else
                    return music[selectedProp] == selectedValue;
            });
        }
        ;
    });
